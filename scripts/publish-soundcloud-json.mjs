...
const BLOG_SOUNDCloudJSON_FILENAME = "soundcloud.json";
...
if (!existsSync(SOUNDCloudJSON_PATH)) {
  console.error(
    "❌ soundcloud.json not found. Run 'npm run generate:music' first.",
  );
  process.exit(1);
}
...
withGitWorktree(BLOG_DIRSTRING, "blog-posts", async (worktreeDir) => {
  const blogSoundCloudJsonPath = resolve(
    worktreeDir,
    BLOG_SOUNDCloudJSON_FILENAME,
  );
  console.log("📋 Copying soundcloud.json to blog worktree...");
  copyFileSync(SOUNDCloudJSON_PATH, blogSoundCloudJsonPath);
});
